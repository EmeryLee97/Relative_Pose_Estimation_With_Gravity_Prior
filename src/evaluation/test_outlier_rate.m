...
% initialization
len_outlier = length(outlier_rate_list);

runtime_ransac_3pts = zeros(len_outlier, iter_times);
runtime_ransac_5pts = zeros(len_outlier, iter_times);
runtime_polynomial_opt = zeros(len_outlier, iter_times);
runtime_gbnb_opt = zeros(len_outlier, iter_times);
runtime_gbnb_sampling = zeros(len_outlier, iter_times);
runtime_gbnb_stabbing = zeros(len_outlier, iter_times);

theta_err_ransac_3pts = zeros(len_outlier, iter_times);
theta_err_ransac_5pts = zeros(len_outlier, iter_times);
theta_err_polynomial_opt = zeros(len_outlier, iter_times);
theta_err_gbnb_opt = zeros(len_outlier, iter_times);
theta_err_gbnb_sampling = zeros(len_outlier, iter_times);
theta_err_gbnb_stabbing = zeros(len_outlier, iter_times);

t_err_ransac_3pts = zeros(len_outlier, iter_times);
t_err_ransac_5pts = zeros(len_outlier, iter_times);
t_err_polynomial_opt = zeros(len_outlier, iter_times);
t_err_gbnb_opt = zeros(len_outlier, iter_times);
t_err_gbnb_sampling = zeros(len_outlier, iter_times);
t_err_gbnb_stabbing = zeros(len_outlier, iter_times);
...
for i = 1:length(outlier_rate_list)
    ...
    for j = 1:iter_times
        ...
        theta_err_ransac_3pts(i, j) = abs(theta_gt - theta_ransac_3pts) * 180 / pi;
        ...
        theta_err_ransac_5pts(i, j) = abs(theta_gt - theta_ransac_5pts) * 180 / pi;
        ...
        theta_err_polynomial_opt(i, j) = abs(theta_gt - theta_polynomial_opt) * 180 / pi;
        ...
        theta_err_gbnb_opt(i, j) = abs(theta_gt - theta_gbnb_opt) * 180 / pi;
        ...
        theta_err_gbnb_sampling(i, j) = abs(theta_gt - theta_gbnb_sampling) * 180 / pi;
        ...
        theta_err_gbnb_stabbing(i, j) = abs(theta_gt - theta_gbnb_stabbing) * 180 / pi;
...
color4 = [246, 193, 77] / 255;
color5 = [109, 49, 135] / 255;
color6 = [18, 80, 123] / 255;

color = {color1; color2; color3; color4; color5; color6};

slg = semilogy(outlier_rate_list, runtime_gbnb_opt, '-diamond', ...
         outlier_rate_list, runtime_gbnb_sampling, '-o', ...
         outlier_rate_list, runtime_gbnb_stabbing, '-pentagram', ...
         outlier_rate_list, runtime_polynomial_opt, '-v', ...
         outlier_rate_list, runtime_ransac_3pts, '-^', ...
         outlier_rate_list, runtime_ransac_5pts, '-s');
...
test_outlier_result.outlier_list = [factor; factor; factor; factor; factor; factor];

test_outlier_result.trans_error = [reshape(t_err_ransac_5pts', [], 1); ...
...
test_outlier_result.rot_error = [reshape(theta_err_ransac_5pts', [], 1); ...
...
test_outlier_result.method = [string(repmat("Ransac\_5pts", iter_times*length(outlier_rate_list), 1)); ...
                            string(repmat("Ransac\_3pts", iter_times*length(outlier_rate_list), 1)); ...
                            string(repmat("Polynomial\_opt", iter_times*length(outlier_rate_list), 1)); ...
                            string(repmat("GBnB\_opt", iter_times*length(outlier_rate_list), 1)); ...
                            string(repmat("GBnB\_sampling", iter_times*length(outlier_rate_list), 1)); ...
                            string(repmat("GBnB\_stabbing", iter_times*length(outlier_rate_list), 1))];

offset = 15 * ones(size(factor));
offset_list = [-2.5*offset; -1.5*offset; -0.5*offset; 0.5*offset; 1.5*offset; 2.5*offset];
test_outlier_result.outlier_list = test_outlier_result.outlier_list + offset_list;

% draw translation error

figure
axesbox = boxchart(test_outlier_result.outlier_list, ...
    test_outlier_result.trans_error, ...
    'GroupByColor', test_outlier_result.method,...
    'WhiskerLineStyle', '--', ...
    'BoxFaceAlpha', 0.6, ...
    'MarkerStyle', 'x', ...
    'MarkerSize', 8, ...
...
% Don't forget to change the xLabel if use another outlier list
...
[theta_err_gcransac, t_err_gcransac, rumtime_gcransac, ...
 theta_err_magsacpp, t_err_magsacpp, runtime_magsacpp] = getErrorOutlier(100);
...
l_theta = plot(outlier_rate_list, succ_rate_theta_gbnb_opt, '-diamond', ...
            outlier_rate_list, succ_rate_theta_gbnb_sampling, '-o', ...
            outlier_rate_list, succ_rate_theta_gbnb_stabbing, '-pentagram', ...
            outlier_rate_list, succ_rate_theta_polynomial, '-v', ...
            outlier_rate_list, succ_rate_theta_ransac_3pts, '-^', ...
            outlier_rate_list, succ_rate_theta_ransac_5pts, '-s', ...
            outlier_rate_list, succ_rate_theta_gcransac, '-square', ...)
            outlier_rate_list, succ_rate_theta_magsacpp, '-hexagram', ...
            'LineWidth', 2);
...
legend('gBnB', 'SaBnB', 'ISBnB', 'OPT', 'RANSAC&3pts', ...
       'RANSAC&5pts', 'GC-RANSAC&5pts', 'MAGSAC++&5pts');

xlabel("Outlier rate");
ylabel("Successful rate (err\_rot<1.5\circ)");
...
l_t = plot(outlier_rate_list, succ_rate_t_gbnb_opt, '-diamond', ...
           outlier_rate_list, succ_rate_t_gbnb_sampling, '-o', ...
           outlier_rate_list, succ_rate_t_gbnb_stabbing, '-pentagram', ...
           outlier_rate_list, succ_rate_t_polynomial, '-v', ...
           outlier_rate_list, succ_rate_t_ransac_3pts, '-^', ...
           outlier_rate_list, succ_rate_t_ransac_5pts, '-s', ...
           outlier_rate_list, succ_rate_theta_gcransac, '-square', ...)
           outlier_rate_list, succ_rate_theta_magsacpp, '-hexagram', ...
           'LineWidth', 2);
...
xlabel("Outlier rate");
ylabel("Successful rate (err\_trans<1.5\circ)");

grid on;

%% recall

recall_list = 0.1:0.2:1.5;
