function [upper_bound, lower_bound, theta] = get_bound_theta_test( ...
    a, b, c, branch_2d, epsilon)
    % Interval merging tool box needed (Version 1.2.0.0 by Bruno Luong)
    % dbstop in get_bound_theta917 at 148 if abs(branch_2d(3)-0.1963)<1e-3
    ...
    gamma = -sqrt(2)*delta : 0.00001 : sqrt(2)*delta; % (1, m)
...
%     k2(idx_k2) = nan;
    ...
    for i = 1:size(theta_min_1, 2)
        [left, right] = IntervalUnion( ...
            [theta_min_1(i), theta_min_2(i), theta_min_3(i), theta_min_4(i)], ...
            [theta_max_1(i), theta_max_2(i), theta_max_3(i), theta_max_4(i)]);
        lower = [lower, left];
        upper = [upper, right];
        for j = 1:length(left)
            plot([left(j), right(j)], [i, i]);
            hold on
        end
    end

    % get upper bound
    [upper_bound, theta] = Max_Stabbing(lower, upper);
    plot([theta, theta], [0, 52]);
